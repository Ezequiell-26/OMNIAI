...
/**
 * ProvidersPanel — pestaña BYOK del centro de ajustes.
 * Migrado desde el antiguo ApiKeyModal (ahora todo vive en SettingsDialog).
 * Valida formato soft, cifra con AES-GCM y persiste solo en IndexedDB.
 */

import { useEffect, useState } from 'react';
import { Eye, EyeOff, ExternalLink, Loader2, ShieldCheck, Trash2 } from 'lucide-react';
...
export function ProvidersPanel() {
  ...
  // Al montar el panel, estado limpio (sin borradores).
  useEffect(() => {
    setDrafts({});
    setVisible({});
  }, []);
  ...
  return (
    <div className="grid gap-3">
      <p className="text-xs leading-relaxed text-muted-foreground">
        Introduce tus propias claves (BYOK). Se cifran con AES-GCM y se guardan únicamente en la
        base de datos local de tu navegador. El servidor las usa en memoria durante cada petición
        y nunca las almacena.
      </p>
          ...
          <div key={provider} className="glass-card rounded-xl p-4">
            ...
            {!formatValid && (
              <p className="mt-1.5 text-[11px] text-destructive/90">
                El formato no coincide con lo esperado ({info.keyHint}) — se guardará de todas
                formas.
              </p>
            )}
